...
//	}
	...
	public String sendPanicMessage(String camID){ // panic message with method to send panic server if pressed twice within 5 seconds
		...
		if((timeDiff + 5000) > currTime){ // uses last time set to compare with new time to clarify the difference
			this.notifyServer(camID); // if time is within 5 seconds, it will call method below
			return"";
		}
	...
	public String notifyServer(String camID){ // this method is called from the if statement
		Timestamp panicTime = new Timestamp(System.currentTimeMillis());
		messageStatus = "Assistence needed " + panicTime + "\n";
		server.panicServer(camID); // calls panic method in the server (regional office class)
...
public class HomeHub extends JFrame {
	...
	public HomeHub(String[] args, String homeHubName2) {
		...
		try {
		    ...
		    homehubClient = new ClientServerHomeHubServant(this, orb);
